              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl shadow-slate-200 dark:shadow-black/50 overflow-hidden border border-slate-100 dark:border-slate-700 relative">
                ...
                <div className="p-6 md:p-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6 md:mb-8">
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800">
                            <p className="text-[10px] md:text-xs text-slate-400 uppercase mb-2">Bus Details</p>
                            <div className="flex flex-col gap-1.5">
                                {(ticket["Bus Number"] || ticket.busNumber) ? (
                                    <div className="flex items-center gap-2">
                                        <span className="px-2 py-0.5 bg-purple-100 text-purple-700 text-[10px] font-bold rounded uppercase tracking-wider">Bus No</span>
                                        <span className="font-bold text-slate-700 dark:text-slate-300 text-sm md:text-base">{ticket["Bus Number"] || ticket.busNumber}</span>
                                    </div>
                                ) : (
                                    <span className="text-sm text-slate-400 italic">Bus No. pending</span>
                                )}
                                {(ticket["Conductor Number"] || ticket.conductorNumber) ? (
                                    <div className="flex items-center gap-2">
                                        <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-[10px] font-bold rounded uppercase tracking-wider">Contact</span>
                                        <span className="font-bold text-slate-700 dark:text-slate-300 text-sm md:text-base">{ticket["Conductor Number"] || ticket.conductorNumber}</span>
                                    </div>
                                ) : (
                                    <span className="text-sm text-slate-400 italic">Contact pending</span>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                {/* Footer */}
                <div className="p-5 md:p-6 bg-slate-50 dark:bg-slate-900/30 grid grid-cols-3 gap-2 items-center">
                     <div>
                        ...
                        <span className={`inline-block px-2.5 py-0.5 md:px-3 md:py-1 rounded-full text-[10px] md:text-xs font-bold ${
                            (ticket.Status || ticket.status || 'Confirmed') === 'Confirmed'

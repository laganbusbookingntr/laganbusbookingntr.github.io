...
interface BookingFormProps {
  ...
  cities: string[];
  blockedBuses?: {bus: string, date: string}[];
}

const BookingForm: React.FC<BookingFormProps> = ({ onSubmit, busServices, cities, blockedBuses = [] }) => {
  ...
  useEffect(() => {
    if (formData.bus && formData.date) {
      const isBlocked = blockedBuses.some(b => b.bus === formData.bus && b.date === formData.date);
      if (isBlocked) {
        setFormData(prev => ({ ...prev, bus: '' }));
        Swal.fire({
          icon: 'warning',
          title: 'Bus Unavailable',
          text: 'The selected bus is fully booked for this date. Please choose another bus or date.',
          toast: true,
          position: 'top-end',
          timer: 4000,
  ...
  }, [blockedBuses, formData.date, formData.bus]);

  const handleBusChange = (busName: string) => {
    const service = busServices[busName];
    setFormData(prev => ({
      ...prev,
      bus: busName,
  ...
  const calculateTotal = () => {
    const service = busServices[formData.bus];
    if (!service) return 0;
        ...
        <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
          ...
          <div className="space-y-4">
             ...
             <div className="grid md:grid-cols-2 gap-4 md:gap-6">
                 ...
                 {/* Show blocked buses messages */}
                 {blockedBuses.filter(b => b.date === formData.date).length > 0 && (
                     <div className="mt-3 p-3 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/50 rounded-xl animate-fade-in-up">
                         <div className="text-xs font-bold text-red-600 dark:text-red-400 mb-1.5 uppercase tracking-wider flex items-center gap-1.5">
                             <div className="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse"></div>
                             Fully Booked Services
                         </div>
                         <div className="flex flex-wrap gap-2">
                             {blockedBuses.filter(b => b.date === formData.date).map((b, idx) => (
                                 <span key={idx} className="text-xs font-medium text-red-700 dark:text-red-300 bg-white dark:bg-slate-900 px-2 py-1 rounded-md shadow-sm border border-red-100 dark:border-red-800/30">
                                     {b.bus}
                                 </span>
                             ))}
                         </div>
